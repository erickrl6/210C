...
%rng(123456,'twister'); % with this one, prod shocks matter to change sign of IRFs, so set at 0
rng(456,'twister');
...
for s = 1:N_sim
    ...
    y4 = zeros(horizon,1);
    for t = 5:horizon
        y4(t) = b4(1) + b4(2:5)' * flipud(y4(t-4:t-1)) + b4(6) * eps_irf(t);
    end
    IRF_4lag(:, s) = y4;
    ...
    y12 = zeros(horizon,1);
    for t = 13:horizon
        y12(t) = b12(1) + b12(2:13)' * flipud(y12(t-12:t-1)) + b12(14) * eps_irf(t);
    end
    IRF_12lag(:, s) = y12;
end
...
legend('1 lag', '4 lags', '12 lags');
xlabel('Time (t)');
ylabel('Impulse Response of y_t');
title('Median Impulse Response from Misspecified Models');
grid on;

% Save figure
saveas(gcf, 'out/3_IRF_misspec_y_v2.png');